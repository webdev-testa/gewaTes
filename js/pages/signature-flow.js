...
function renderYoonaInputs(item, sIdx, iIdx) {
  ...
  const flowers = [
    { name: "Lily", emoji: "🌸" }, // Using generic flower emoji if no image
    { name: "Anthurium", emoji: "🌿" }
  ];
  ...
  flowers.forEach(f => {
    ...
    html += `
      <div class="flower-choice-card ${isSelected}" 
        ...
        <span class="flower-choice-emoji">${f.emoji}</span>
        <div>${f.name}</div>
      </div>
    `;
  });
...
function getSignatureReviewHTML(item, size, isEdit, pIdx, sIdx, iIdx) {
  ...
  let html = `<div style="font-weight:bold; margin-bottom:4px;">${sName}</div>`;
